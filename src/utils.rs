...
macro_rules! any_box {
    ($val : expr) => {
        $crate::prelude::Box::new($val) as $crate::prelude::Box<dyn core::any::Any>
    };
}
...
pub mod test_utils {
    ...
    macro_rules! load_program {
        ($file:expr, $entrypoint: expr $(,)?) => {
            $crate::types::program::Program::from_bytes(include_str!($file).as_bytes(), $entrypoint)
                .unwrap()
        };
    }
    ...
    macro_rules! load_program {
        ($file:expr, $entrypoint: expr $(,)?) => {
            $crate::types::program::Program::from_file(
    ...
    macro_rules! memory_inner {
        ($mem:expr, ($si:expr, $off:expr), ($sival:expr, $offval: expr)) => {
            ...
            while matches!(res, Err(MemoryError::UnallocatedSegment(_, _))) {
                $mem.data.push(crate::vec::Vec::new());
                res = $mem.insert(k, v);
            }
        };
    ...
    macro_rules! references {
        ($num: expr) => {{
            let mut references = crate::collections::HashMap::<usize, HintReference>::new();
            for i in 0..$num {
    ...
    macro_rules! program {
        ...
        ( $( $builtin_name: expr ),* ) => {
            Program {
                ...
                data: crate::vec::Vec::new(),
                constants: crate::collections::HashMap::new(),
                main: None,
                start: None,
                end: None,
                hints: crate::collections::HashMap::new(),
                reference_manager: ReferenceManager {
                    references: crate::vec::Vec::new(),
                },
                identifiers: crate::collections::HashMap::new(),
                error_message_attributes: crate::vec::Vec::new(),
        ...
        ( $( $name: expr ),* ) => {
            {
                ...
                let mut ids_data = crate::collections::HashMap::<crate::string::String, HintReference>::new();
                for (i, name) in ids_names.iter().enumerate() {
                    ids_data.insert(crate::string::ToString::to_string(name), references.get(&i).unwrap().clone());
                }
                ids_data
            }
        };
        ...
        ( $( ($name: expr, $offset:expr) ),* ) => {
            {
                let mut ids_data = crate::collections::HashMap::<crate::string::String, HintReference>::new();
                $(
                    ids_data.insert(crate::string::String::from($name), HintReference::new_simple($offset));
                )*
                ids_data
            }
        };
    ...
    macro_rules! run_hint {
        ...
        ($vm:expr, $ids_data:expr, $hint_code:expr, $exec_scopes:expr) => {{
            let hint_data = HintProcessorData::new_default(
                crate::string::ToString::to_string($hint_code),
                $ids_data,
            );
            let mut hint_processor = BuiltinHintProcessor::new_empty();
    ...
    macro_rules! dict_manager {
        ($exec_scopes:expr, $tracker_num:expr, $( ($key:expr, $val:expr )),* ) => {
            ...
            $exec_scopes.insert_value("dict_manager", crate::rc::Rc::new(core::cell::RefCell::new(dict_manager)))
        };
...
#[cfg(test)]
mod test {
    use crate::{cell::RefCell, collections::HashMap, rc::Rc, string::String, vec::Vec};
    use crate::{
        hint_processor::{
